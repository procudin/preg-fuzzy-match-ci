...
if ($ADMIN->fulltree) {
    ...
    $settings->add(new admin_setting_configselect('qtype_preg/defaultengine', get_string('defaultenginelabel', 'qtype_preg'),
                    get_string('defaultenginedescription', 'qtype_preg'), 'fa_matcher', $engines));

    $notations = $qtypeobj->available_notations();
    $settings->add(new admin_setting_configselect('qtype_preg/defaultnotation', get_string('defaultnotationlabel', 'qtype_preg'),
                    get_string('defaultnotationdescription', 'qtype_preg'), 'native', $notations));

    $a = get_string('objectname', 'qtype_preg');
    $settings->add(
        new block_formal_langs_admin_setting_language(
            'qtype_preg/defaultlang',
            get_string('defaultlanglabel', 'block_formal_langs'),
            get_string('defaultlangdescription', 'block_formal_langs', $a),
            '1',
            null
        )
    );

    $settings->add(new admin_setting_configtext('qtype_preg/maxerrorsshown', get_string('maxerrorsshownlabel', 'qtype_preg'),
    ...
    $settings->add(new admin_setting_configtext('qtype_preg/fa_state_limit', get_string('fa_state_limit', 'qtype_preg'),
                    get_string('fa_state_limit_description', 'qtype_preg'), 250, PARAM_INT));

    $settings->add(new admin_setting_configtext('qtype_preg/fa_transition_limit', get_string('fa_transition_limit', 'qtype_preg'),
                    get_string('fa_transition_limit_description', 'qtype_preg'), 250, PARAM_INT));

    $settings->add(
        new admin_setting_configtext(
            'qtype_preg/fa_simulation_state_limit',
            get_string('fa_simulation_state_limit', 'qtype_preg'),
            get_string('fa_simulation_state_limit_description', 'qtype_preg'),
            2000,
            PARAM_INT
        )
    );
    ...
    $assertionsupport = [
        '0' => get_string('assertfailmodeasis', 'qtype_preg'),
        '1' => get_string('assertfailmodemerge', 'qtype_preg'),
    ];
    $settings->add(new admin_setting_configselect('qtype_preg/assertfailmode', get_string('assertfailmodelabel', 'qtype_preg'),
